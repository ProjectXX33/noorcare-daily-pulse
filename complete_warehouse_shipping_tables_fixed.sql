-- Complete Warehouse & Shipping Management System Tables (FIXED VERSION)
...
DO $$
BEGIN
    -- Add foreign key for shipped_by if it doesn't exist
    BEGIN
        ALTER TABLE order_submissions 
        ADD CONSTRAINT fk_order_submissions_shipped_by 
        FOREIGN KEY (shipped_by) REFERENCES users(id);
    EXCEPTION
        WHEN duplicate_object THEN NULL;
    END;
    
    -- Add foreign key for shipping_method if it doesn't exist
    BEGIN
        ALTER TABLE order_submissions 
    ...
    RAISE NOTICE 'Order submissions table updated successfully';
END $$;
...
-- Shipping methods policies (with proper error handling)
DO $$
BEGIN
    -- Drop and recreate shipping_methods_read policy
    BEGIN
    ...
    -- Drop and recreate shipping_methods_admin_manage policy
    BEGIN
...
-- Order notes policies (with proper error handling)
DO $$
BEGIN
    BEGIN
    ...
    RAISE NOTICE 'Order notes policies created successfully';
END $$;

-- Order status history policies (with proper error handling)
DO $$
BEGIN
    BEGIN
    ...
    RAISE NOTICE 'Order status history policies created successfully';
END $$;

-- Shipping tracking events policies (with proper error handling)
DO $$
BEGIN
    BEGIN
    ...
    RAISE NOTICE 'Shipping tracking events policies created successfully';
END $$;

-- Warehouse performance metrics policies (with proper error handling)
DO $$
BEGIN
    BEGIN
    ...
    RAISE NOTICE 'Warehouse performance metrics policies created successfully';
END $$;

-- Order submissions warehouse access (with proper error handling)
DO $$
BEGIN
    BEGIN
    ...
    RAISE NOTICE 'Order submissions warehouse access policy created successfully';
END $$;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Could not assign sample data - this is normal if no orders exist yet';
END $$;
...
BEGIN
    ...
    RAISE NOTICE '  🚚 SMSA Express (2-day delivery)';
    RAISE NOTICE '  🚛 DRB Logistics (3-day delivery)';
    RAISE NOTICE '  🏪 Our Shipped (1-day delivery)';
    RAISE NOTICE '  📦 Standard Shipping (5-day delivery)';
    RAISE NOTICE '';
...
-- =====================================================
-- 8. TABLE REPLICA IDENTITY
-- =====================================================

-- Set replica identity to FULL for tables that need it for RLS/triggers
ALTER TABLE public.order_submissions REPLICA IDENTITY FULL;

-- =====================================================
-- 9. STORED PROCEDURES & FUNCTIONS
-- =====================================================

-- Update existing WooCommerce orders to have the raw status
UPDATE order_submissions 
